...
export function ImageEditor() {
  ...
  const handleDownload = useCallback(async (options: DownloadOptionsType) => {
    if (!image || !crop) {
      toast({
        title: "Fel",
    ...
    try {
      ...
      const blob = await canvasToBlob(canvas, options.fileType, options.quality / 100);
      
      const link = document.createElement("a");
      link.download = options.useOriginalFilename 
        ? uploadedFile?.name || generateFilename({
            width: canvas.width,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  <Tabs defaultValue="preset" value={activeTab} onValueChange={setActiveTab}>
                    <TabsList className="grid w-full grid-cols-2">
                      <TabsTrigger value="preset">Förinställda format</TabsTrigger>
          ...
          <div className="space-y-6">
            ...
            <DownloadOptions
              ...
              initialType={uploadedFile?.type === "image/png" ? "image/png" : "image/jpeg"}
              cropWidth={activeTab === "custom" ? customWidth : (crop?.width || 0)}
              cropHeight={activeTab === "custom" ? customHeight : (crop?.height || 0)}
